...
import { checkServiceExits, getCurrentLocation, removeObjectWithId } from "../../../utils/helpers/halper";
...
const ServiceDetails = ({ navigation, route }) => {
  ...
  const status = cartData?.items?.some((itm) => itm?.service_id === itemDetails?.id);
  ...
  const createOrUpdateCart = async () => {
    if (cartData?.cart_id) {
      if (status) {
        const filteredItem = cartData?.items?.filter((item) => item?.service_id === itemDetails?.id)[0];
  ...
  useEffect(() => {
    if (isFocused && ServiceReducer.status == "Service/getServiceDetailsSuccess") {
  ...
  function LoadingSkeleton() {
    return (
      <SkeletonPlaceholder borderRadius={4}>
        {[1].map((item, index) => {
          return (
            <SkeletonPlaceholder.Item key={index} width={"95%"} alignSelf="center" marginTop={normalize(8)} justifyContent="space-between">
              <SkeletonPlaceholder.Item width={normalize(290)} height={170} borderRadius={8} alignSelf="center" />
              <SkeletonPlaceholder.Item width={normalize(290)} height={30} borderRadius={8} alignSelf="center" marginVertical={10} />
              <SkeletonPlaceholder.Item width={normalize(290)} height={170} borderRadius={8} alignSelf="center" />
              <SkeletonPlaceholder.Item flexDirection="row" marginVertical={normalize(16)}>
          ...
          <View>
            ...
            >
              ...
              >
                ...
                <TouchableOpacity onPress={createOrUpdateCart} style={styles.touch}>
                  {!status && <Image style={styles.addImg} source={Icons.add} />}
                  <Text
                    style={[
                      styles.addTitle,
                      {
        ...
        <ImageView
          ...
          FooterComponent={({ imageIndex }) => (
            <>
              ...
              >
                <Image source={Icons.iconVertical} style={[styles.icon, { tintColor: "white" }]} />
              </TouchableOpacity>

              <ImageFooter imageIndex={imageIndex} imagesCount={productDetails?.image_urls?.length} />
            </>
          )}
        />

        <Modal isVisible={imagesVertical} style={{ padding: 0, margin: 0 }} onBackdropPress={() => setImagesVertical(false)}>
          <View
            style={{
              flex: 1,
          ...
          <View style={{ position: "absolute", bottom: 30, alignSelf: "center" }}>
            ...
            >
              <Image source={Icons.icon_photo_swipe} style={[styles.icon, { tintColor: "white" }]} />
            </TouchableOpacity>
          </View>
        </Modal>
        {/* Prithick */}
