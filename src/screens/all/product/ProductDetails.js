...
import { addToCartProductRequest, getProductDetailsRequest, removeCartProductItemsRequest } from "../../../redux/reducer/ProductReducer";
...
const ProductDetails = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (isFocused && ProductReducer.status == "Product/getProductDetailsSuccess") {
  ...
  const createOrUpdateCart = async () => {
    if (cartData?.cart_id) {
      if (status) {
        const filteredItem = cartData?.items?.filter((item) => item?.product_id === itemDetails?.id)[0];
        ...
        <ImageView
          ...
          FooterComponent={({ imageIndex }) => (
            <>
              ...
              <ImageFooter imageIndex={imageIndex} imagesCount={productDetails?.gallery_images?.length} />
            </>
          )}
        />
