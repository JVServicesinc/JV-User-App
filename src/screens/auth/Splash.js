...
const Splash = () => {
  ...
  function languageChanges() {
    try {
      ...
      const _findex = _dfLan?.findIndex(
        item => item.languageTag == defaultLnaguage,
      );
      ...
      if (_findex != -1) {
        ...
        if (defaultLnaguage?.length > 1) {
          ...
          const _findex2 = _dfLan?.filter(
            item => item.languageCode == defaultLnaguage,
          );
